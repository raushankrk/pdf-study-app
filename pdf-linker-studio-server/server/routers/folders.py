"""Folders REST API (project-scoped)."""
import time
import os
...
def _get_child_folders(parent_id: str, project_id: str) -> list[dict]:
    if parent_id is None or parent_id == "root":
        rows = db.query_all(
            "SELECT * FROM folders WHERE project_id = ? AND (parent_id IS NULL OR parent_id = 'root') ORDER BY name",
            (project_id,)
        )
    else:
        rows = db.query_all(
...
def _get_descendant_ids(folder_id: str, project_id: str) -> list[str]:
    out = []
    stack = [folder_id]
    while stack:
        current = stack.pop()
        children = _get_child_folders(current, project_id)
        for c in children:
...
@router.get("")
def list_folders(project_id: str = Depends(get_current_project)):
    rows = db.query_all(
        "SELECT * FROM folders WHERE project_id = ? ORDER BY name", (project_id,)
    )
    return [_folder_row_to_dict(r) for r in rows]


@router.get("/tree")
def get_tree(project_id: str = Depends(get_current_project)):
    def build(parent_id):
        children = _get_child_folders(parent_id, project_id)
        for c in children:
...
@router.post("")
def create_folder(folder: FolderCreate, project_id: str = Depends(get_current_project)):
    ...
    sibs = _get_child_folders(folder.parent_id, project_id)
    ...
    fid = f"folder_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def rename_folder(folder_id: str, update: FolderUpdate, project_id: str = Depends(get_current_project)):
    if folder_id == "root":
        raise HTTPException(400, "Root folder cannot be renamed")
    folder = db.query_one(
        "SELECT * FROM folders WHERE id = ? AND project_id = ?", (folder_id, project_id)
    )
    if not folder:
        raise HTTPException(404, "Folder not found")
    if update.name is None or not update.name.strip():
        raise HTTPException(400, "Name cannot be empty")
    sibs = [s for s in _get_child_folders(folder["parent_id"], project_id) if s["id"] != folder_id]
    if any(s["name"].lower() == update.name.lower() for s in sibs):
...
def move_folder(folder_id: str, req: FolderMove, project_id: str = Depends(get_current_project)):
    if folder_id == "root":
    ...
    descendants = _get_descendant_ids(folder_id, project_id)
    if target in descendants:
        raise HTTPException(400, "Cannot move a folder into one of its own descendants")

    folder = db.query_one(
        "SELECT name FROM folders WHERE id = ? AND project_id = ?", (folder_id, project_id)
    )
    if not folder:
        raise HTTPException(404, "Folder not found")

    target_sibs = _get_child_folders(target, project_id)
...
def set_expanded(folder_id: str, expanded: bool, project_id: str = Depends(get_current_project)):
    if folder_id == "root":
        return {"status": "ok"}
    db.execute(
...
def delete_folder(folder_id: str, move_contents_to_root: bool = False, project_id: str = Depends(get_current_project)):
    if folder_id == "root":
    ...
    docs = db.query_all(
        f"SELECT id FROM documents WHERE project_id = ? AND folder_id IN ({placeholders})",
        (project_id, *all_folder_ids)
    )

    if move_contents_to_root:
        children = _get_child_folders(folder_id, project_id)
        for c in children:
            db.execute(
                "UPDATE folders SET parent_id = NULL WHERE id = ? AND project_id = ?",
                (c["id"], project_id)
            )
        for d in docs:
            db.execute(
                "UPDATE documents SET folder_id = 'root' WHERE id = ? AND project_id = ?",
                (d["id"], project_id)
            )
        db.execute(
            "DELETE FROM folders WHERE id = ? AND project_id = ?", (folder_id, project_id)
        )
    else:
        from .documents import _delete_document_completely
        for d in docs:
            _delete_document_completely(d["id"], project_id)
